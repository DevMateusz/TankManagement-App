...
const loginUserToAccount = async(req, res) => {
  try {
    ...
    const user = await User.findOne({ email: lowerCaseEmail }, { _id: 1, fullName: 1, email: 1, password: 1});
    ...
    if(!user) {
      ...
      return res.status(200).json({ _id: user._id, fullName: user.fullName, email: user.email, token: token });
    };
